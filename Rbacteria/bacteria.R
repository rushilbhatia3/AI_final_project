...
indata <- read.csv("colTable.csv", header=FALSE)			#read the input file into indata
...
numElems <-as.integer(nrow(indata)/k)		 #find the size of each fold  
print(numElems)
...
for(i in 1:k)	#loop once for each fold
{
  ...
  temp <- data[(i*numElems):nrow(indata), ]			#pull out everything after the test data
  ...
  print("model done")
  #print(fit)
  plot(fit, uniform=TRUE, main="CART Data")	#plot the model
  text(fit, use.n=TRUE, all=TRUE, cex=1.8)				                        #make it look a bit nicer
  ...
  for(j in 1:nrow(predFrame))					#loop over each prediction
  {
    ...
    print(sprintf("classdata[rownum]: %s", classData[rowNum,]))
